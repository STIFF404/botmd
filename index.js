    ...
    naze.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, fileLength: jumhal, contextInfo: { mentionedJid: [num] }, caption: `Hay kak @${num.split("@")[0]}\nWelcome To ${metadata.subject}\n\n_Semoga Betah Ya Kak_`, buttons: buttons, footer: nyoutube})
                } else if (anu.action == 'remove') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, fileLength: jumhal, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Keluar/Dikick Dari ${metadata.subject}`, buttons: buttons, footer: nyoutube})
                } else if (anu.action == 'promote') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, fileLength: jumhal, mentions: [num], caption: `@${num.split('@')[0]} Selamat Anda Jadi Atmin ${metadata.subject}`, buttons: buttons, footer: nyoutube})
                } else if (anu.action == 'demote') {
                    naze.sendMessage(anu.id, { image: { url: ppuser },fileLength: jumhal,  mentions: [num], caption: `@${num.split('@')[0]} Cie Turun Jabatan ${metadata.subject}`, buttons: buttons, footer: nyoutube})
              }
            }
        } catch (err) {
            console.log(err)
